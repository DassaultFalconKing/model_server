...
TEST_F(HttpRestApiHandlerTest, V3ChatCompletionsPropagatesSessionIdAndGenericHeadersIntoRequestComponents) {
    std::unordered_map<std::string, std::string> inbound{
        {"content-type", "application/json"},
        {"X-OVMS-Session-ID", "gemma4-live-test"},
        {"X-OVMS-Test-Passthrough", "diagnostic-value"},
    };
    ...
    auto passthrough = components.headers.find("X-OVMS-Test-Passthrough");
    ASSERT_NE(passthrough, components.headers.end());
    EXPECT_EQ(passthrough->second, "diagnostic-value");
}
